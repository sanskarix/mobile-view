  ...
  return <div className="p-0 max-w-none mx-auto space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center">
          ...
          <input type="text" value={formData.url} onChange={e => handleFormChange('url', e.target.value)} className="flex-1 px-4 py-3 border border-border focus:ring-2 focus:ring-ring focus:border-transparent bg-background text-sm text-gray-600 h-10" />
          <div className="flex items-center border border-l-0 border-border rounded-r-lg bg-background h-10">
            <button onClick={handleCopyUrl} title="Copy URL" className="p-3.5 transition-colors border-r border-border bg-[#f1f5f9] hover:bg-[#f1f5f9]/80 rounded-none h-10 flex items-center justify-center">
              <Copy className="h-4 w-4 text-gray-600" />
            </button>
            <button onClick={handlePreviewUrl} className="p-3 hover:bg-muted transition-colors rounded-r-lg bg-[#f1f5f9] hover:bg-[#f1f5f9]/80 h-10 flex items-center justify-center" title="Preview">
      ...
      <div>
        ...
        {/* Duration Input Field with Inline Tags */}
        <div className="relative" ref={durationDropdownRef}>
          <div className="w-full min-h-[40px] px-3 py-2 border border-border rounded-lg focus-within:ring-2 focus-within:ring-ring bg-background cursor-text flex flex-wrap items-center gap-2" onClick={() => setFormData(prev => ({
          ...prev,
          showDurationDropdown: !prev.showDurationDropdown
        }))}>
            {/* Selected Duration Tags */}
            {formData.durations.map(duration => <span key={duration} className="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-700 text-sm rounded border">
                {duration} mins
                <button onClick={e => {
              e.stopPropagation();
            ...
            {formData.durations.length === 0 && <span className="text-gray-400 text-sm">Select durations...</span>}
            
            {/* Dropdown Arrow */}
            <div className="ml-auto">
              {formData.showDurationDropdown ? <ChevronUp className="h-4 w-4 text-gray-400" /> : <ChevronDown className="h-4 w-4 text-gray-400" />}
            </div>
          </div>
          
          {/* Recommendations Dropdown */}
          {formData.showDurationDropdown && <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-border rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
              {getRecommendedDurations().map(duration => <button key={duration} onClick={() => addDuration(duration)} className="w-full text-left px-3 py-2 hover:bg-gray-50 text-sm">
                  {duration} mins
                </button>)}
              {getRecommendedDurations().length === 0 && <div className="px-3 py-2 text-sm text-gray-500">
      ...
      <div>
        ...
        <CustomSelect value={formData.defaultDuration} onValueChange={value => handleFormChange('defaultDuration', value)} options={defaultDurationOptions} placeholder="Select default duration" />
      </div>
      ...
      <div>
        ...
        <CustomSelect value={formData.location} onValueChange={value => handleFormChange('location', value)} options={locationOptions} placeholder="Select location" />
