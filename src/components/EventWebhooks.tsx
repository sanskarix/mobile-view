          ...
          <div className="space-y-3">
            {webhooks.map(webhook => <div key={webhook.id} className="p-4 border border-gray-200 rounded-lg bg-white">
                ...
                <div className="flex flex-wrap gap-2">
                  {webhook.events.slice(0, 3).map(event => <span key={event} className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-full">
                      {event}
                    </span>)}
                  {webhook.events.length > 3 && <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-full">
