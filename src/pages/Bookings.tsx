...
export default function Bookings() {
  ...
  const MeetingCard = ({
    ...
    return <TooltipProvider>
        <div className={`bg-card border border-border rounded-lg p-6 hover:shadow-md transition-shadow ${isHost ? 'cursor-pointer' : ''}`} onClick={() => {
        if (isHost) {
          setExpandedMeeting(isExpanded ? null : meeting.id);
        }
      }}>
                    ...
                    {attendeeDisplay && <div className="relative">
                        ...
                        {showAttendeesDropdown === meeting.id && typeof attendeeDisplay === 'object' && <div className="absolute top-full left-0 mt-1 bg-popover border border-border rounded-md shadow-lg z-50 min-w-48">
                            <div className="p-2">
                              ...
                              {meeting.attendees.map((attendee, index) => <Tooltip key={index}>
                                  <TooltipTrigger asChild>
                                    <button className="w-full text-left text-sm text-foreground hover:bg-accent hover:text-accent-foreground py-1 px-2 rounded" onClick={e => {
                              e.stopPropagation();
  ...
  return <div className="p-6 space-y-6">
      ...
      <div className="border-b border-border">
        <nav className="-mb-px flex space-x-8">
          ...
          key: 'recurring',
          label: 'Recurring',
          count: recurringMeetings.length
        }, {
          key: 'past',
          label: 'Past',
          ...
          key: 'canceled',
          label: 'Canceled',
          count: canceledMeetings.length
        }].map(tab => <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors hover:border-gray-300 ${activeTab === tab.key ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
              {tab.label} ({tab.count})
            </button>)}
        </nav>
      </div>
      ...
      {/* Filter Panel */}
      {showFilters && <div className="bg-card border border-border rounded-lg p-4">
          <div className="grid grid-cols-4 gap-4 mb-4">
                ...
                <PopoverTrigger asChild>
                  <Button variant="outline" className="w-full justify-start text-left font-normal">
                    ...
                    {dateRange?.from ? dateRange.to ? <>
                        ...
                        </> : dateRange.from.toLocaleDateString() : <span>Pick a date range</span>}
                  </Button>
                </PopoverTrigger>
            ...
            <div>
              ...
              <select className="w-full px-3 py-2 border border-input bg-background text-foreground rounded-md" value={filteredAttendee} onChange={e => setFilteredAttendee(e.target.value)}>
                <option value="All">All</option>
                {getAllAttendees().map(attendee => <option key={attendee} value={attendee}>{attendee}</option>)}
              </select>
            </div>
            
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">Event Type</label>
              <select className="w-full px-3 py-2 border border-input bg-background text-foreground rounded-md" value={filteredEventType} onChange={e => setFilteredEventType(e.target.value)}>
                <option value="All">All</option>
            ...
            <div>
              ...
              <select className="w-full px-3 py-2 border border-input bg-background text-foreground rounded-md" value={filteredTeam} onChange={e => setFilteredTeam(e.target.value)}>
                <option value="All">All</option>
      ...
      {/* Today's Meetings */}
      {activeTab === 'upcoming' && todayMeetings.length > 0 && <div>
          <h2 className="font-medium text-foreground mb-4 text-xs">Today</h2>
          <div className="space-y-4">
      ...
      {/* Other Meetings */}
      <div className="space-y-4">
        {(activeTab === 'upcoming' ? otherMeetings : currentMeetings).map(meeting => <MeetingCard key={meeting.id} meeting={meeting} />)}
      </div>

      {/* Add Guests Modal */}
      <AddGuestsModal isOpen={showAddGuests} onClose={() => setShowAddGuests(false)} onAdd={(emails: string[]) => {
      toast({
