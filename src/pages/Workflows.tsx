...
import { Plus, Workflow, ChevronDown, Copy, ExternalLink, Download, Code, MoreHorizontal, Mail, MessageSquare, MessageCircle, Edit } from 'lucide-react';
  ...
  const getActionIcons = (actions: string[]) => {
    return actions.map((action, index) => {
      if (action.toLowerCase().includes('email')) {
        return <Mail key={index} className="h-3 w-3 text-muted-foreground" />;
      } else if (action.toLowerCase().includes('sms')) {
        return <MessageSquare key={index} className="h-3 w-3 text-muted-foreground" />;
      } else if (action.toLowerCase().includes('whatsapp')) {
        return <MessageCircle key={index} className="h-3 w-3 text-green-600" />;
      }
      return null;
    });
  };
    ...
    <div className="bg-card border border-border rounded-lg p-6 hover:border-border/60 transition-all hover:shadow-sm">
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          {/* Actions line - showing all added actions */}
          <p className="text-muted-foreground text-sm mb-3">
            {formatActions(workflow.actions || [workflow.action])}
          </p>
...
export const Workflows = () => {
  ...
  // Load workflows from localStorage and check for new ones
  useEffect(() => {
    const loadWorkflows = () => {
      ...
      if (loadedWorkflows.length === 0) {
        loadedWorkflows = [{
          ...
          actions: ['Send email to attendees', 'Send SMS to host'],
          enabled: true,
          ...
          actions: ['Send email to host', 'Send thank you email to attendees'],
          enabled: true,
          ...
          actions: ['Send SMS to attendees', 'Send WhatsApp to host'],
          enabled: false,
          ...
          action: 'Send emails to host',
          actions: ['Send email to host'],
          enabled: true,
          eventTypeInfo: 'Active on 2 event types',
          teamName: 'Development Team',
          createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString()
        }];
        localStorage.setItem('workflows', JSON.stringify(loadedWorkflows));
      }
    ...
    // Listen for storage changes to update workflows when new ones are added
  ...
  const handleWorkflowEdit = (workflowId: string) => {
    ...
    // Navigate to workflow builder with pre-filled data instead of modal
